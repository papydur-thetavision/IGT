...
class CatheterDataModule(pl.LightningDataModule):
    ...
    def __init__(self, volume_reader: VolumeReader, batch_size: int = 16):
        ...
        self.volume_reader.precompute_endpoints()
        self.batch_size = batch_size
        self.vrs = {'train': (self.volume_reader.get_train_val_reader())[0],
                    'val': (self.volume_reader.get_train_val_reader())[1]}

    def train_dataloader(self):
        return DataLoader(self.vrs['train'], batch_size=self.batch_size, num_workers=self.config.num_workers, pin_memory=True, shuffle=True)

    def val_dataloader(self):
        return DataLoader(self.vrs['val'], batch_size=self.batch_size, num_workers=self.config.num_workers, pin_memory=True)


class Catheter3DSystem(pl.LightningModule):
    def __init__(self, lr=1e-3, monitor='val_loss'):
        super().__init__()
        self.lr = lr
        self.model = Instrument3DModel(base_channels=16)
    ...
    def configure_optimizers(self):
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=0.001, pct_start=0.2,
                                                        total_steps=500, verbose=True, three_phase=True)
